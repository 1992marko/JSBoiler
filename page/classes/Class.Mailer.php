...
if($_POST['mode'] == "kontakt"){
	$myMessage = '<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	</head>
		<body>
			<h3>Upit sa web stranice od '.$_POST["ime"].', '.$_POST['email'].'</h3>
			<p>'.$_POST["message"].'</p>
		</body>
	</html>';

	$dummy = new Mailer();
	$dummy->setFrom("www.zagreb-touristinfo.hr", $_POST['from']);
	$dummy->addRecipient("TZ Zagreb", "info@infozagreb.hr");
	$dummy->addRecipient("I. Gudelj", "ivan@borming.hr");
	$dummy->fillSubject("Upit sa web stranice (".date("d.m.Y H:i:s").")");
	$dummy->fillMessage($myMessage);
	$dummy->send();
}

else if($_POST['mode'] == "newsletter"){
	$timePassed = time() - $_SESSION['session_time'];
	if($timePassed > 60){
		...
		<head>
			...
			<body>
				<p>Korisnik '.$_POST['email'].' se želi prijaviti na newsletter!</p>
			</body>
		</html>';
	
		$_SESSION['session_time'] = time();
		$dummy = new Mailer();
		$dummy->setFrom("www.zagreb-touristinfo.hr", $_POST['email']);
		$dummy->addRecipient("Jelena Anić", "jelena.anic@infozagreb.hr");
		$dummy->addRecipient("Petra Maršić", "pmarsic@zagreb-touristinfo.hr");
		$dummy->addRecipient("Ivan Gudelj", "ivan@borming.hr");
		$dummy->fillSubject("PRIJAVA newsletter | Korisnik: ".$_POST['email']." | Jezik: ".$_POST['lang']." | (".date("d.m.Y H:i:s").")");
		$dummy->fillMessage($myMessage);
		$dummy->send();
		echo "true";
	}
...
else if($_POST['mode'] == "newsletter-unsubscribe"){
	$timePassed = time() - $_SESSION['session_time'];
	if($timePassed > 60){
		...
		<head>
			...
			<body>
				<p>Korisnik '.$_POST['email'].' se želi odjaviti sa newslettera!</p>
			</body>
		</html>';
		...
		$dummy->addRecipient("Petra Maršić", "pmarsic@zagreb-touristinfo.hr");
		$dummy->addRecipient("Ivan Gudelj", "ivan@borming.hr");
		$dummy->fillSubject("ODJAVA newsletter | Korisnik: ".$_POST['email']." | Jezik: ".$_POST['lang']." | (".date("d.m.Y H:i:s").")");
		$dummy->fillMessage($myMessage);
		$dummy->send();
		echo "true";
	}
